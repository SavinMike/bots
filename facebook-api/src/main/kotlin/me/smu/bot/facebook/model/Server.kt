...
import me.smu.bot.facebook.model.network.api.gson.PageEntityTypeAdapter
import me.smu.bot.facebook.model.network.api.gson.ServerRequestTypeAdapter
import me.smu.bot.model.logger.DefaultLogger
import me.smu.bot.model.logger.Logger
import org.slf4j.event.Level

internal class Server(webhook: Webhook,
                      verifyToken: String,
        ...
        server = embeddedServer(factory = webhook.applicationEngineFactory, port = webhook.port, host = webhook.host) {
            ...
            install(ContentNegotiation) {
                gson {
                    ...
                    registerTypeAdapter(PageEntry::class.java, PageEntityTypeAdapter)
                    registerTypeAdapter(ServiceRequest::class.java, ServerRequestTypeAdapter)
                }
            }
            routing {
                get(path = "webhook") {
                ...
                post("/webhook") {
                    ...
                    try {
                        val body = call.receive<ServiceRequest<*>>()
                        logger.log(message = "test Webhook Post")
